...
def create_access_token_subadmin(data: dict):
    ...
    encoded_jwt = jwt.encode(to_encode, SECRET_KEY_SUBADMIN, algorithm=ALGORITHM)
...
def verify_access_token_subadmin(token:str, Credentials_exception ):
    try:
        payload = jwt.decode(token,SECRET_KEY_SUBADMIN,algorithms=[ALGORITHM])
        id:str = payload.get("user_id")
...
def get_current_subadmin(token:str = Depends(oauth2_scheme),db: Session = Depends(database.get_db)):
    ...
    user = db.query(models.subadmin).filter(models.subadmin.id == token.id).first()
