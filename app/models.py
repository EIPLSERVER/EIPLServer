...
class subadmin(Base):
    ...
    password =Column (String, nullable= False)
    created_by=Column(Integer ,ForeignKey("admin.id", ondelete="CASCADE"), nullable= False)
...
class slots(Base):
    ...
    phone = Column(BigInteger,ForeignKey("user.phone", ondelete="CASCADE") , nullable= False )
    bay_id= Column(Integer,ForeignKey("bay.id", ondelete="CASCADE"), nullable= False)
    product_id =Column (Integer,ForeignKey("products.id", ondelete="CASCADE"), nullable= False)
    client_id =Column (Integer, ForeignKey("clients.id", ondelete="CASCADE"),nullable= False)
...
class days_10(Base):
    ...
    id = Column(Integer, primary_key=True, nullable= False)
    bay_id = Column(Integer,ForeignKey("bay.id", ondelete="CASCADE"),nullable= False)
    date=Column(Date,ForeignKey("month.date", ondelete="CASCADE"),nullable=False)
