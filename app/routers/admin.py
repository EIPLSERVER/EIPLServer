...
def get_subadmins(phone: int, db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_admin)):
    user = db.query(models.subadmin).filter(
        models.subadmin.phone == phone).first()

    if not user:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail=f"subadmin with phone: {phone} not found")
...
async def slot_60create(slot: schemas.slot_60, db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_admin)):
    ...
    # created_by = current_user.id,**user.dict())
    new_slot = models.slot_60(**slot.dict())
    db.add(new_slot)
    db.commit()
    db.refresh(new_slot)
    return new_slot
...
async def slot_30create(slot: schemas.slot_30, db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_admin)):
    ...
    # created_by = current_user.id,**user.dict())
    new_slot = models.slot_30(**slot.dict())
    db.add(new_slot)
    db.commit()
    db.refresh(new_slot)
    return new_slot
...
def get_slots_60(db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_admin)):
    ...
    if not user:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"No slots found")
...
async def month_create(slot: schemas.month, db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_admin)):
    ...
    # created_by = current_user.id,**user.dict())
    new_slot = models.month(**slot.dict())
    db.add(new_slot)
    db.commit()
    db.refresh(new_slot)
    return new_slot
...
def month(db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_admin)):
    ...
    if not user:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"No dates found")
...
async def days_10(db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_admin)):
    ...
    new_slot = db.query(models.month.date, models.bay.id, models.slot_30.slot).filter(
        models.bay.product_id == 1, models.month.working == True).order_by(models.month.date).all()
    new_slot = new_slot + db.query(models.month.date, models.bay.id, models.slot_60.slot).filter(
        models.bay.product_id != 1, models.month.working == True).order_by(models.month.date).all()
