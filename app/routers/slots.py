...
async def posts(db: Session = Depends(get_db), limit: int = 10, skip: int = 0, search: Optional[str] = ""):
    print(limit)
    post = db.query(models.slots).filter(models.slots.truck.contains(
...
async def filled(product: str, date: date, db: Session = Depends(get_db)):
    post = []
    product_id = db.query(models.products.id).filter(
        models.products.product == product).all()
    bay = db.query(models.bay.id).filter(
        models.bay.product_id == product_id[0][0]).all()
    for i in bay:
        post = post + db.query(models.days_10.id, models.days_10.bay_id, models.days_10.slot, models.days_10.slot).filter(
            models.days_10.bay_id == i[0], models.days_10.date == date, models.days_10.booked == False).all()
    return post
...
async def screate(payload: schemas.createslot, slot_id: schemas.createslot_id, db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_user)):
    ...
    new_post = models.slots(phone=current_user.phone,
                            client_id=current_user.client_id, **payload.dict())
    post_update = db.query(models.days_10).filter(
        models.days_10.id == slot_id.slot_id).first()
