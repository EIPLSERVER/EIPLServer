...
async def delete_bay(id : int ,db: Session = Depends(get_db) ,current_user : int = Depends(oauth2.get_current_admin)):
    bay = db.query(models.clients_products).filter(models.clients_products.id == id)
    bay_q=bay.first()
...
async def update_bay(id:int,user:schemas.clients_products,db: Session = Depends(get_db) ,current_user : str = Depends(oauth2.get_current_admin)):
    bay_q = db.query(models.clients_products).filter(models.clients_products.id == id)
    bay=bay_q.first()
...
def get_clients_products(client:str,db: Session = Depends(get_db), current_user : int = Depends(oauth2.get_current_admin)):
    client = db.query(models.clients.id).filter(models.clients.client== client).first()
    print(client.id)
    products_id = db.query(models.clients_products.product_id).filter(models.clients_products.client_id == client.id).all()
    print(products_id)
